 ...
 */
export type SchemaOf<T extends Version<any, any>> = T extends Version<infer S, any> ? S : never
...
export class VersionedEntity<LatestVer extends number, M extends Record<LatestVer, Version<any, any>> & Record<number, Version<any, any>>> {
  ...
  public safeParse(data: unknown): ParseResult<SchemaOf<M[LatestVer]>> {
    ...
    if (!pass.success) {
      return { type: "err", error: { type: "GIVEN_VER_VALIDATION_FAIL", version: ver, versionDef: verDef, error: pass.error } }
    }
    ...
    for (let up = ver + 1; up <= this.latestVersion; up++) {
      ...
      if (!upDef) {
        return { type: "err", error: { type: "BUG_NO_INTERMEDIATE_FOUND", missingVer: up } }
      }

      if (upDef.initial) {
        return { type: "err", error: { type: "BUG_INTERMEDIATE_MARKED_INITIAL", ver: up } }
      }
...
export function createVersionedEntity<
  ...
  VersionMap extends Record<LatestVer, Version<any, any>> & Record<number | LatestVer, Version<any, any>>,
>(def: {
